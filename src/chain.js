...
class Chain {
  ...
  _initGenesis() {
    ...
    try {
      ...
      this.db.prepare(`INSERT INTO blocks (height, hash, parent_hash, timestamp, miner, challenge_id, tx_root, nonce, difficulty, target,
        ...
        String(totalFees), gasUsed, 30000000, genesis.base_target
      );
      this.altura = 0;
  ...
  addBlock(bloco, opts = {}) {
    ...
    // Bug: this summed each tx's gas_limit (the cap), not the gas actually consumed,
    // so gas_used always equalled the sum of limits regardless of real usage — every
    // block looked maximally full. Until real per-tx gas accounting exists, the honest
    // minimum reflects the base transfer cost (21000) per tx rather than the caller-supplied limit.
    const gasUsed = txs.reduce((s, t) => s + 21000, 0);
  ...
  // Validate a single incoming tx against current confirmed state (signature, nonce, balance)
  // BEFORE it's allowed into the mempool. This is what prevents a bad tx from later blocking
  // block forging entirely (a bad tx inside a block causes the whole block to be rejected).
  validateTxForMempool(tx) {
    const sender = tx.from_addr;
    ...
    if (!verifySignature(canonicalTxMessage(tx), sig, pubkey.public_key_ed25519)) return { ok: false, motivo: `invalid tx signature from ${sender}` };
    const user = this.db.prepare('SELECT balance, nonce FROM users WHERE address = ?').get(sender);
    const curNonce = user ? safeInt(user.nonce, 0) : 0;
    const curBalance = user ? safeBigInt(user.balance, 0n) : 0n;
    // Allow nonce >= current confirmed nonce (mempool may hold a short queued sequence);
    // exact ordering is still enforced at block-forge time by _validateTxOrder.
  ...
  reorganize(targetOrHash, forceSync) {
    ...
    if (target.height >= this.altura && this.getBlock(target.height) && this.getBlock(target.height).hash === target.hash) {
      return { ok: true, motivo: 'already at tip', height: this.altura, hash: this.melhorHash };
    }
  ...
  _purgeOrphanedBlocks() {
    ...
    let cur = this.db.prepare('SELECT hash, parent_hash, height FROM blocks WHERE hash = ?').get(this.melhorHash);
    while (cur) {
    ...
    if (purged > 0) {
      log('info', `Purged ${purged} orphaned blocks, chain now height=${this.altura} tip=${this.melhorHash.slice(0, 10)}`);
    }
    return purged;
  }
