...
class ChallengeManager {
  ...
  getOrCreate() {
    ...
    const baseTarget = this.chain._baseTargetForHeight(this.chain.altura);
    ...
    const ttl = this.chain.altura === 0 ? Math.max(20, this.cfg.expectedTimePerBlock || 240) : Math.max(minTtl, 86400);
  ...
  submitProof(chain, challengeId, miner, plotId, deadline, proofPacket = null) {
    ...
    const parentBlock = chain.getBlock(ch.block_height || chain.altura);
    const networkBaseTarget = chain._baseTargetForHeight(ch.block_height || chain.altura);
    ...
    if (updated.changes > 0) {
      ...
      const deadlineElapsed = now >= (ch.created_at || 0) + deadline;
      if (deadlineElapsed) {
        log('info', `Deadline ${deadline}s elapsed — forging block for challenge ${challengeId.slice(0, 12)}`);
        ...
        if (block) {
          ...
          if (addResult.ok) {
            ...
            log('error', `Immediate block forge rejected for challenge ${challengeId.slice(0, 12)}: ${addResult.motivo}`);
          }
        }
      } else {
        const waitSec = (ch.created_at || 0) + deadline - now;
  ...
  _selectValidMempoolTxs(chain, maxCount) {
    ...
    for (const tx of candidates) {
      ...
      if (!reason) {
        ...
        else if (safeInt(tx.nonce, 0) !== projectedNonce[sender]) reason = 'transactions not ordered by nonce';
      ...
      if (reason) {
        log('warn', `Dropping bad mempool tx ${(tx.hash || '').slice(0, 12)}: ${reason}`);
  ...
  finalizeExpiredChallenges(chain, syncEngine) {
    ...
    const stale = this.db.prepare(`SELECT challenge_id, block_height FROM mining_challenges
      ...
      AND (created_at + winner_deadline) <= ? AND block_height < ?`).all(now, chain.altura);
    for (const s of stale) {
      log('warn', `Abandoning stale challenge ${s.challenge_id.slice(0, 12)} (created at height ${s.block_height}, chain now at ${chain.altura})`);
    ...
    const readyToForge = this.db.prepare(`SELECT * FROM mining_challenges WHERE forged_block_height IS NULL
      ...
      AND (created_at + winner_deadline) <= ? AND block_height >= ?
      ORDER BY created_at ASC LIMIT 1`).get(now, chain.altura);
    if (readyToForge) {
      ...
      } else if (chain.altura >= nextHeight) {
        this.db.prepare('UPDATE mining_challenges SET forged_block_height = ? WHERE challenge_id = ? AND forged_block_height IS NULL').run(chain.altura, ch.challenge_id);
      } else {
  ...
  _forgeBlockForChallenge(chain, syncEngine, challenge) {
    try {
      ...
      for (const { sub, score } of scores) {
        ...
        if (reward > 0n) {
          distribution.push({
            miner: sub.miner, plot_id: sub.plot_id || '', size_gb: sub.size_gb,
      ...
      const result = chain.addBlock(block, { skipStateValidation: true, skipPocValidation: true, skipSignature: true, skipTxValidation: true });
      if (!result.ok) {
